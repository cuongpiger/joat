...
func GetRandomName(retry int, psep string) string {
	if psep == "" {
		psep = "_"
	}
begin:
	name := left[rand.Intn(len(left))] + psep + right[rand.Intn(len(right))] //nolint:gosec // G404: Use of weak random number generator (math/rand instead of crypto/rand)
